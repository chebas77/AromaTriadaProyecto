    ...
    public function agregarServicios(Request $request)
{
    ...
    if ($request->has('servicios')) {
        $serviciosSeleccionados = $request->input('servicios'); // Array de IDs de servicios
        $cantidadMozos = $request->input('cantidad_mozos', 1); // Cantidad de mozos
        ...
        foreach ($serviciosSeleccionados as $servicioId) {
            ...
            if ($servicio) {
                ...
                // Manejar el caso del servicio "Mozo"
                if (strtolower($servicio->nombre) === 'mozo') {
                    $carrito[$key] = [
                        'id' => $servicio->id_servicio,
                        'nombre' => $servicio->nombre,
                        'cantidad' => $cantidadMozos, // Tomar la cantidad seleccionada
    ...
    return redirect()->route('carrito.mostrar')->with('error', 'No se seleccionaron servicios.');
}
